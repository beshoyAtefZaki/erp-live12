...
class SalaryStructureAssignment(Document):
	...
	def on_update(self):
		...
		total_hours = to_timedelta(shift_type.end_time) - to_timedelta(shift_type.start_time)
		if total_hours:
			total_sec = frappe.db.sql(
					"""select TIME_TO_SEC(%s) from `tabSalary Structure Assignment` where name = %s""",[total_hours,self.name])[0][0]
			self.total_min = total_sec / 60
