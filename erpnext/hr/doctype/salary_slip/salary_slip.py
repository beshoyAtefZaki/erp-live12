...
class SalarySlip(TransactionBase):
    ...
    def on_update(self):
        self.get_attendance_deduction()
        if frappe.db.exists("Sales Person", {"employee": self.employee}):
            sales_person = frappe.db.get_value("Sales Person", {"employee": self.employee}, "name")
            self.commission = frappe.db.sql("""select sum(st.incentives) from `tabSales Invoice` as si,`tabSales Team` as st where si.name = st.parent and
								si.status = "Paid" and si.docstatus = 1 and  si.posting_date >= %s and si.posting_date <= %s and st.sales_person = %s"""
                                            , [self.start_date, self.end_date, sales_person])[0][0]
        self.earnings = []
        self.deducations = []
        self.calculate_net_pay()

    def get_attendance_deduction(self):
        from frappe.utils import to_timedelta
        self.attendance_present = frappe.db.sql("""select count(name) from `tabAttendance` where status = "Present" and employee = %s and 
        												attendance_date >= %s and attendance_date <= %s and docstatus = 1"""
                                                , [self.employee, self.start_date, self.end_date])[0][0]
        self.attendance_absent = frappe.db.sql("""select count(name) from `tabAttendance` where status = "Absent" and employee = %s and 
        												attendance_date >= %s and attendance_date <= %s and docstatus = 1"""
                                               , [self.employee, self.start_date, self.end_date])[0][0]
        if frappe.db.exists("Attendance Deduction",
                            {"posting_date": [">=", self.start_date], "posting_date": ["<=", self.end_date],
                             "docstatus": 1}):
            rate = frappe.db.sql(
                """select sum(rate_minutes) from `tabAttendance Deduction` where absent = 0 and docstatus = 1 and posting_date >= %s and posting_date <= %s"""
                , [self.start_date, self.end_date])[0][0]
            absent = frappe.db.sql(
                """select sum(rate) from `tabAttendance Deduction` where absent = 1 and docstatus = 1 and posting_date >= %s and posting_date <= %s"""
                , [self.start_date, self.end_date])[0][0]

            self.total_minutes = rate
        ...
        for struct_row in self._salary_structure_doc.get("deductions"):
            if struct_row.variable_based_on_taxable_salary == 1 and not struct_row.formula and not struct_row.amount:
                ...
                if tax_detail and tax_detail[1]:
                    self.update_component_row(frappe._dict(tax_detail[0]), tax_detail[1], "deductions", tax_detail[2],
                                              tax_detail[3])
        ...
        if payroll_period:
            ...
            if (getdate(payroll_period.end_date) <= getdate(self.end_date)):
                ...
                last_benefits = get_last_payroll_period_benefits(self.employee, self.start_date, self.end_date, \
                                                                 current_flexi_amount, payroll_period,
    ...
    def add_employee_flexi_benefits(self, struct_row):
        if frappe.db.get_value("Salary Component", struct_row.salary_component, "pay_against_benefit_claim") != 1:
            benefit_component_amount = get_benefit_component_amount(self.employee, self.start_date, self.end_date, \
                                                                    struct_row, self._salary_structure_doc,
                                                                    self.total_working_days, self.payroll_frequency)
            if benefit_component_amount:
                self.update_component_row(struct_row, benefit_component_amount, "earnings")
        else:
            benefit_claim_amount = get_benefit_claim_amount(self.employee, self.start_date, self.end_date,
    ...
    def on_submit(self):
        if self.net_pay < 0:
            ...
            if (frappe.db.get_single_value("HR Settings",
                                           "email_salary_slip_to_employee")) and not frappe.flags.via_payroll_entry:
    ...
    def calculate_variable_tax(self, tax_component, payroll_period):
        ...
        if self.deduct_tax_for_unclaimed_employee_benefits or self.deduct_tax_for_unsubmitted_tax_exemption_proof:
            ...
            if tax_detail:
                pro_rata_tax_paid = tax_detail["total_tax_paid"] - tax_detail["additional_tax"] - tax_detail[
                    "benefit_tax"]
    ...
    def get_tax_paid_in_period(self, payroll_period, tax_component, only_total=False):
        ...
        if sum_tax_paid and sum_tax_paid[0][0]:
            return {'total_tax_paid': sum_tax_paid[0][0], 'benefit_tax': sum_tax_paid[0][1],
