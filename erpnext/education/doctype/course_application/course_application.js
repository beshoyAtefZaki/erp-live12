...
frappe.ui.form.on('Course Application', {
...
frappe.ui.form.on('Course Application', {
	refresh: function(frm)
	     {frm.fields_dict['waiting_list'].get_query = function(doc, cdt, cdn) {
			return {
				filters: [
					['Waiting List', 'parent1', '=', doc.course],
					['Waiting List','available','>',0],
					['Waiting List','course_start_date','>=',doc.date],
	    ...
	    {frm.fields_dict['student_applicant'].get_query = function(doc, cdt, cdn) {
          	return {
				filters: [
                    ...
                    ['Student Applicant','application', '=', 'Course']
...
frappe.ui.form.on("Course Application","has_placement_test",function(frm){
...
frappe.ui.form.on("Course Application","down_payment",function(frm){
...
frappe.ui.form.on("Course Application", "amount", function(frm) {
	frm.set_df_property("amount", "read_only", frm.doc.__islocal ? 0 : 1);
});
frappe.ui.form.on("Course Application", "percent", function(frm) {
	frm.set_df_property("percent", "read_only", frm.doc.__islocal ? 0 : 1);
});
frappe.ui.form.on("Course Application", "item", function(frm) {
	frm.set_df_property("item", "read_only", frm.doc.__islocal ? 0 : 1);
});
**/
frappe.listview_settings['Course Application'] = {
