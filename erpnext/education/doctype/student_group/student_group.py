...
class StudentGroup(Document):
	...
	def validate(self):
		...
		for d in self.students:
			d.parent_course = self.course_parent
	...
	def on_update(self):
		for d in self.students:
			wl = frappe.get_doc("Waiting List", {"student_group": self.name})
			if not d.done and d.application == "Course" and not frappe.db.exists("Course Application", {"waiting_list": wl.name,
																"applicaiton_status": "Admitted", "student": d.student}):
				d.done = 1
				wl.actual += 1
