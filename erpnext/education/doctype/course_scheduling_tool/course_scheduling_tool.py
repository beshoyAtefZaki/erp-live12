...
class CourseSchedulingTool(Document):
	...
	def make_course_schedule(self, date):
		...
		course_schedule.company = self.company
	...
	def update_student_group(self):
		...
		sg.instructors = []
		#sg.days = []
		sg.append("instructors", {"instructor": self.instructor})
		sg.append("days", {"days": self.day})
		sg.from_date = self.course_start_date
		sg.to_date = self.course_end_date
		sg.from_time = self.from_time
		sg.to_time = self.to_time
		sg.company = self.company
		sg.save()
