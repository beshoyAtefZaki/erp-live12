...
class StudentApplicant(Document):
	...
	def validate(self):
		...
		if frappe.db.exists("Academic Term", {"term_start_date": ["<=", self.application_date], "term_end_date": [">=", self.application_date]}):
			term = frappe.get_doc("Academic Term", {"term_start_date": ["<=", self.application_date], "term_end_date": [">=", self.application_date]})
			self.academic_year = term.academic_year
			self.academic_term = term.name
		else:
			frappe.throw("Please Set Academic Year / Term For Date {0}".format(self.application_date))
